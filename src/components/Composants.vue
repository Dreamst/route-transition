...
export default {
  ...
  mounted() {
    this.anim_composants
      .to(".box.composants", {border: "1px solid black", duration: .2})
      .to(".typo", { duration: 1, x: 1000, ease: "power2.out" })
      .to(".header", {height: 0, opacity: 0, duration: .5})
      .to(".box.composants", {
        duration: .5,
        width: "calc(100% - 40px)",
        ease: "power2.in"
      }, "<+.5")
      .to(".box.composants", {position: 'relative', marginTop: 0, left: 0, duration: 0, top: -30})
     ...
     // Defines animation for content reveal and plays it
      this.anim_content
      .fromTo(".composants-content", { height: "0", display: 'block'},{ height: "auto", y: 50, duration: 1, delay: 1.2 })
      .to(".composants-content", {opacity: 1, y: 0, duration: .5})
      this.anim_content.play();
  },
  beforeDestroy() {
    this.anim_composants.reverse();
  }
